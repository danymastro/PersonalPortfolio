      ...
      <div className="p-6 sm:p-8 space-y-6">
        ...
        <div className="pt-5 border-t-2 border-black/10 flex flex-wrap items-center justify-between gap-4">
          ...
          <button
            ...
            className="group flex items-center gap-2 px-5 py-2.5 bg-black text-white rounded-xl text-sm font-bold hover:bg-[#2563EB] transition-colors cursor-pointer"
          >
    ...
    <section id="problems" className="relative py-24 sm:py-32 px-4 sm:px-8 bg-black text-[#FFFDF5] overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 sm:mb-16 gap-6">
          ...
          <div className="flex items-center gap-2 p-1.5 bg-white/10 border border-white/20 rounded-2xl shrink-0">
            <button
              onClick={() => setViewMode('terminal')}
              className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-mono font-bold transition-all cursor-pointer ${
                viewMode === 'terminal'
                  ? 'bg-[#D0FF71] text-black neo-shadow-sm'
            ...
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-mono font-bold transition-all cursor-pointer ${
                viewMode === 'preset'
                  ? 'bg-[#FDE047] text-black neo-shadow-sm'
                  : 'text-white/70 hover:text-white'
              }`}
            >
              <ListFilter className="w-3.5 h-3.5" />
              <span>{isIt ? 'Problemi Frequenti' : 'Common Issues'}</span>
            </button>
          </div>
        </div>
                ...
                return (
                  <div key={item.id}>
                    <button
                      ...
                      className={`w-full text-left p-5 rounded-2xl border-2 transition-all duration-300 group cursor-pointer ${
                        isActive
